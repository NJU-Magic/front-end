...
class SingleModalProcessReconstruction extends Component {
    ...
    _submit = async (title) => {
        ...
        if (title === 'rgb') {
            ...
            let data = {"algorithm": "PSGN", 'method_name': "psgn", 'input_path': this.state.selected_data_rgb.data_url, 'Sensor_ID': this.state.selected_data_rgb.number};
            ...
            if(res.success === 1) {
                ...
                let temp_item = {
                    number: this.state.selected_data_rgb.number,
                    data_url: res.result_path,
                    descript: this.state.selected_data_rgb.descript,
                    date: this.state.selected_data_rgb.date,
                    data_type: "点云",
                }
                this.setState({
                    // data_url: item.data_url,
                    visible_data_drawer: false,
                    selected_data_rgb_output: temp_item })
            } else {
                message.error("处理失败")
            }
            ...
            console.log(this.state.selected_data_rgb);
            let data = {"algorithm": "PCN", 'method_name': "pcn", 'input_path': this.state.selected_data_pc.data_url, 'Sensor_ID': this.state.selected_data_pc.number};
            let res = await algorithmCall(data);
            console.log(res)
    ...
    showComponent=(item, bim_show_sufix)=>{
        ...
        if(data_type==="点云"){
            return(
                <BIMShow div_id={item.number+bim_show_sufix+"process_recon"} model_url={item.data_url} height={144} width={256}/>
            )
        }
    };
    ...
    showComponent2=(item, bim_show_sufix)=>{
        ...
        if(data_type==="点云"){
            return(
                <BIMShow div_id={bim_show_sufix+"process_recon"} model_url={item.data_url} height={250} width={400}/>
            )
        }
    };
...
//                                     </div>
//                                     <div>
//                                         <Button onClick={() => this.select_data_from_database()} type="primary"
//                                                 className="button_selectdata">从数据库中选择</Button>
//
//                                     </div>
//                                     <div>
